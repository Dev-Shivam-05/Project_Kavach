...
**✚ ADDS (F-21) — notification content policy.** Push payloads are **data-only, Class B/C exclusively**: `{incident_id, family_id, trigger, tier, subject_short_name, kind, owner_short_name}`. The device composes human-readable text locally from decrypted state. iOS uses a **Notification Service Extension** with keychain access to decrypt before display. Lock-screen preview shows short name + trigger class only.

> **`kind` and `owner_short_name` were added on 11 Aug (W10-d · 1.32).** The original five could not express *what a push is about*, so §2.6.4's requirement that CLAIM fan out over both channels was unbuildable: a claim delivered over push would have been read as a second emergency and rung the alarm stream at the exact moment this document says to stop ringing. `kind` is `alert | claimed | released` and defaults to `alert` on anything unrecognised — the fail-safe direction, since a claim shown as an alert costs one wasted siren and an alert shown as a banner costs the alert. `owner_short_name` is emitted **only** on a claim, is the same ASCII short name already permitted for the subject, and is what makes "Rohan is responding" possible instead of "someone is responding" — which is the diffusion of responsibility §2.6.4 exists to prevent. Neither field is inferable from duress: a claim happens identically on duress and non-duress incidents. Enforced fail-closed by `assertPushSafe` in `backend/internal/notify/fcm.go` and read through an allowlist again on the device in `mobile/src/state/pushReceive.ts`.
